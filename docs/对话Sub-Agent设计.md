...
- 面板分为本次 Run 的子 Agent 列表和单个子 Agent 详情，两个视图不同时展示。思考链条目直接打开对应详情；点击详情头部的返回按钮进入列表，再从列表选择其他子 Agent。列表按 `parentExecutionId` 将孙 Agent 收进直接父 Agent 的可展开分组，并在父行显示子任务数量；缺失父记录的历史条目仍作为顶层项可访问。思考链和右侧列表中的子孙 Agent 保持相同左侧起点，不添加层级缩进或结构线；思考链中的孙 Agent 保留委派来源。列表使用与文字行高协调的小头像，展示任务名、角色与状态。
- 详情头部以任务名为首行，次行显示内联状态、角色及可选的委派来源；长任务名和来源单行省略，并可悬停查看完整内容。返回按钮、头像与尾部操作垂直居中，头部保留上下内距且不被正文挤压；运行中提供“停止该子任务”。关闭面板返回原对话位置，不切换“对话 / 轨迹 / 文件”视图。
