...
| Tool/Skill Registry | `tools/lib/tool-registry.ts` / `tools/lib/skill-registry.ts` | 稳定 ID、schema、来源和权限分类校验 |
...
Tool/Skill 语义选择使用“先权限范围过滤，再 RAG 召回，最后加载完整定义”的两阶段流程。基础工具始终可用；检索超时时回退到基础集，不阻断整个 run。`run.started.data` 记录当次选中的 Tool/Skill ID，便于恢复与审计。
