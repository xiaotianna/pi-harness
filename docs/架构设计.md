...
PATCH  /api/sessions/:sessionId/model             # 修改后续 run 使用的模型，不影响当前活动 run
...
`workspaces` 使用 daemon 通过操作系统目录选择器取得并经 `realpath` 校验的 `root_path` 去重，并保存可编辑的 `display_name`、软移除时间 `removed_at` 与展示顺序 `sort_order`；工作区顺序由 daemon 写入本地 SQLite，不使用浏览器 `localStorage`。Web 创建 Session 时只提交 `workspace_id`，不提供或覆盖真实根目录。移除 Workspace 只隐藏引用及其 Session 列表，不删除本地文件、Session 索引或 JSONL；再次添加同一 `root_path` 时清除 `removed_at`、复用原 `workspace_id`，原会话随之恢复。`sessions` 保存 `workspace_id`、`provider_id`、`model_id`、`title`、`last_seq`、`archived_at` 和时间戳。Session 创建后不修改 `workspace_id`；模型只影响后续新 Run；归档只更新轻量索引，不删除 Session JSONL。`app_settings` 以稳定 key/value 保存所有 Session 共用的审批策略等 daemon 应用设置，不使用浏览器 `localStorage`。
