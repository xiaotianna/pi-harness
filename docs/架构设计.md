...
- 每次调用 `streamFn` 前追加 `context.usage_snapshot`，仅持久化该次请求的 System Prompt、Tool 定义和对话消息 Token 估算值及请求序号，不保存 Context 原文；Web 始终取最新一条快照，分类值不跨模型请求累加。
...
- 当前上下文快照与 Run、Session 的跨请求累计用量是三种独立口径，避免把一个 Run 内因 Tool Call 或自动续轮发生的多次请求相互叠加成窗口占用。
