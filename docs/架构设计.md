...
POST   /api/sessions                              # 创建并绑定不可变 workspaceRoot 与初始模型
...
`workspaces` 使用规范化后的 `root_path` 去重；`sessions` 保存 `workspace_id`、`provider_id`、`model_id`、`title`、`last_seq`、`archived_at` 和时间戳。Session 创建后不修改 `workspace_id`，模型只影响后续新 run；归档只更新轻量索引，不删除 Session JSONL。
