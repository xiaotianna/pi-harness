...
每个 Skill 是以 Skill 名称命名的目录，必须包含 `SKILL.md`。其 YAML frontmatter 必须包含 `name` 和 `description`，目录名与 `name` 一致；只接受 `name`、`description`、`license`、`allowed-tools` 和 `metadata`。可选资源使用 `agents/`、`scripts/`、`references/`、`assets/` 或 Skill 明确引用的其他文件，加载时仍需执行真实路径和大小检查。

Runtime 在每次 Run 前有界读取 workspace 根目录的 `AGENTS.md`，发现两级 Skill，并只将名称、description 和 scope 注入 System Prompt。Agent 根据用户意图进行语义选择，不在 Runtime 中做关键词预加载；确定使用后通过以下内置工具渐进加载：

```text
find_skill  get_skill  load_skill  skill_creator
```

消息输入器的“+ → Skills”通过 Workspace Skill 目录接口展示当前可用的真实 Skill。选中后在编辑器中保持标签展示，提交时序列化为 `$skill-name`，作为显式调用要求 Agent 优先执行 `load_skill`。
...
GET    /api/workspaces/:workspaceId/skills        # 当前 Workspace 可用的项目与全局 Skill 元数据目录
