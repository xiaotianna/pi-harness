...
内置 Provider 的注册顺序由 `load-built-in-provider.ts` 中的有序加载表统一维护；同一服务的全球版与 CN 版必须相邻，Web 与 daemon 不再各自维护另一份顺序。当前 `anthropic`、`xai`、`kimi-coding`、`github-copilot`、`openrouter` 和 `openai-codex` 接入其 `pi-ai` OAuth 实现。Provider 列表通过 `supportsOAuth` 告知 Web 是否显示 OAuth 登录入口，不在 Web 硬编码 Provider ID。
