...
POST   /api/sessions                              # 通过 workspaceId 绑定已登记 Workspace 与初始模型
GET    /api/sessions/:sessionId                   # Session 元数据与可恢复事件快照
PATCH  /api/sessions/:sessionId                   # 空闲时重命名或归档 Session
PATCH  /api/sessions/:sessionId/model             # 空闲时修改后续 run 使用的模型
