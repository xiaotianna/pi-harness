...
| 基础组件 | HeroUI v3 + 仓库内 `@agile-avocation/ui-pro` 兼容包 |
| 业务动画 | Motion |
...
│   │
│
├── hero-ui-pro/              # 本地 vendored HeroUI Pro workspace package
│   ├── dist/                 # 已安装版本的发布产物
│   └── package.json
│
├── package.json
...
`PI_HARNESS_GITHUB_CLIENT_ID` 与 `PI_HARNESS_GITHUB_CLIENT_SECRET`。daemon 的 dev/start
脚本会读取该文件；需要改变端口或 Web 地址时，同时调整 callback URL 与对应环境变量。
开发脚本会强制将 SQLite 数据写入项目根目录的 `.pi-harness/harness.sqlite`，该目录已被
Git 忽略。非 dev 启动仍可通过 `PI_HARNESS_DATABASE_PATH` 指定数据位置；未指定时使用用户
目录下的默认位置。
