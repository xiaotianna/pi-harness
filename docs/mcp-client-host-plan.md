...
> 创建日期：2026-09-10。最后更新：2026-09-12。
...
| 当前任务 | MCP-018/019/037：工具选择、持久授权与连接生命周期 |
| 当前任务状态 | 能力目录按配置版本持久化、启动/新增/重新启用/手动刷新、Run 本地快照装配、工具级启停、可信只读分类与精确参数持久授权已接入；Provider 预算和浏览器验收仍待完善 |
| 核心任务统计 | 共 44 项：已完成 17、进行中 12、待开始 15、阻塞 0 |
| 扩展任务统计 | 共 3 项，均待规划，不计入核心完成率 |
| 当前产出 | 设置 → MCP 服务器；SQLite 能力目录、后台与单台刷新、连接 loading、工具级启停/可信只读；Run 从本地目录冻结工具，执行时复用 Session Client；逐次/持久授权、调用与来源快照 |
| 下一任务 | 补齐 Provider 工具预算、完整结果与恢复，再推进 OAuth 真实服务矩阵、资源/模板及其 UI |
...
- 工具实际执行时按 Session 按需创建并复用 Client；Run 准备不建立连接或重扫目录。无租约连接保留到失效、daemon 关闭或达到 32 个 Client 上限时作为空闲项回收。请求取消优先按调用处理，必须强杀进程时只影响该实例并标记同实例受影响请求。
...
- 分页与自动聚合都有数量、总字节和时间上限；目录按服务器配置版本作为可重建 JSON 持久化到 SQLite，由 daemon 启动、单台新增完成信任、重新启用或手动刷新更新，不在每次 Run 或工具调用前强制重扫。
...
OAuth 入口只在服务器已启用后开放；用户点击后即明确允许该次鉴权发现与授权端点访问。启动时仍校验服务启用、配置版本和凭据版本。同一服务器在 Web 中只保留一个活动授权窗口，重复操作聚焦现有窗口；授权完成保留该配置已有的连接信任，未信任配置不会因此自动变为可信。
...
- OAuth 采用 SDK 支持的发现、PKCE、state、issuer/resource 绑定与凭据存储接口；本地 daemon 以 `token_endpoint_auth_method: none` 注册 public client，预注册/CIMD 为主，DCR 兼容旧服务器。刷新串行，身份或 issuer 变化使旧授权失效，注销清理缓存和连接。
- 凭据位于独立 MCP Credential Store；SQLite 只存非敏感配置和引用。手动录入值只能经写入接口提交一次，不返回明文或放浏览器持久存储。私有文件权限不等于加密，存储级别应如实说明。
- Web 只接收鉴权类型、是否已有凭据及脱敏的凭据版本；新 OAuth 流程必须以凭据版本变化作为完成信号，不能把旧凭据误判为本次授权成功。
- 仅启停或改名且连接配置未变化时，保留凭据并把 `configRevision` 同步到新配置版本；地址、传输、安全项或其他连接配置变化时仍先删除旧凭据，禁止向新目标发送旧 Token。
...
| 工具、资源、模板、提示词与服务信息目录 | SQLite `mcp_catalogs`；按服务器配置版本保存的可重建 JSON 派生数据 |
...
| MCP-009 | Client 生命周期、隔离键、限额与关闭管理 | MCP-006, MCP-007, MCP-008 | 已完成 | Session Client 在 Run 间常驻，32 实例上限时回收空闲项；22 项历史探测覆盖租约复用/Session 隔离、管理连接定向回收、全局撤销、迟到连接中止和幂等关闭；本轮类型检查通过 |
...
| MCP-014 | capability 发现、分页、缓存、目录订阅管理 | MCP-013 | 已完成 | Client 独享目录并由 `list_changed` 失效，不再按 30 秒 TTL 或每 Run 强制刷新；16 页上限、数量/大小限额；12 项历史目录通知/缓存失效/重新订阅探测通过；具体资源 URI 订阅留在 027 实现 |
...
| MCP-018 | 每次 Run 工具准备、选择、预算与冻结 | MCP-017 | 进行中 | 每 Run 从 SQLite 持久目录准备并冻结，只注册设置中启用的工具，不连接或重新发现 MCP；64 工具及 512 KiB 定义预算；Provider 预算适配和运行时验收待完成 |
| MCP-019 | MCP 工具 Policy、持久授权和 Plan 限制 | MCP-008, MCP-017 | 进行中 | 未分类工具独立于 full_access；已接入定义指纹绑定的可信只读分类，以及 workspace/账号/配置/工具定义/精确参数绑定的持久授权；Plan 仅放行可信只读工具；撤销矩阵与运行时验收待完成 |
...
| ID | 任务及交付物 | 依赖 | 状态 | 完成证据 / 验收要求 |
|---|---|---|---|---|
| MCP-025 | OAuth discovery、注册与授权回调 | MCP-007, MCP-011, MCP-013 | 进行中 | 已接入 SDK discovery、DCR、PKCE/state、issuer/resource 校验、浏览器回调和 10 分钟过期；预注册/CIMD、取消与真实服务矩阵待补 |
| MCP-026 | Token 刷新、账号隔离、注销与认证诊断 | MCP-025, MCP-024 | 进行中 | 已按服务串行 refresh；显式 OAuth 流程保留当前配置已有连接信任，未信任配置不会自动提权；远端注销、scope/issuer 变化和清理失败矩阵待补 |
...
| MCP-037 | MCP 设置管理 UI | MCP-015, MCP-024, MCP-026 | 进行中 | 已有全局设置入口、新增弹窗表单/JSON Tab、标准 `mcpServers` 原子导入、编辑、鉴权与 OAuth；启动与生命周期事件自动刷新目录，列表和详情展示 loading，详情可手动刷新并逐工具启停和标记可信只读；完整浏览器验收待补 |
...
| OAuth 与静态凭据 | 007, 025–026 | discovery、登录、刷新、撤销、issuer/resource 和错误路径 | 类型检查通过；真实服务矩阵未执行 |
...
- 本轮修正已完成：MCP 全局共用（含迁移/API/运行时）、添加弹窗 HeroUI 结构，以及表单/JSON Tab 与标准 `mcpServers` 原子导入。MCP-037 已补鉴权自动检测与 OAuth 操作，仍有导入预览、工具权限和完整浏览器验收等剩余工作，任务总体状态保持进行中。
- MCP 列表已改为 Switch 启停；详情展示测试发现的完整目录和服务元数据，响应在进入 Web 前递归移除已知凭据。远程鉴权默认在首次连接时自动检测；OAuth discovery/DCR/PKCE/回调/刷新与 Token/API Key 提示已接入，远端注销和真实服务矩阵仍由 MCP-025/026 跟踪。
- daemon 启动后以 4 个后台 worker 刷新已启用且受信任服务器；新增单台完成信任、重新启用或详情“刷新状态”只刷新该服务器。完整目录按配置版本保存到 SQLite，设置页轮询 loading 状态并从本地 API 读取目录。
- Agent Run 只从持久化目录装配并冻结工具，不连接或重新发现 MCP；工具实际执行时才按 Session 懒连接并复用 Client。配置/凭据/信任变化关闭旧 Client并使旧版本目录失效；空闲连接只在达到 32 Client 上限时回收。
- 工具详情已接入逐工具启停和定义指纹绑定的可信只读分类；未分类工具审批可保存 workspace、账号、配置、定义和精确参数绑定的持久授权，工具设置、信任、配置或身份变化后失效。
...
- 后续顺序：Provider 工具预算 → 多模态/审计/恢复 → OAuth 真实服务矩阵与完整协议 UI → 第三方服务和生产验收。
...
| 容量与回收（历史） | 43 | 连续失败不占用名额、32 Client 上限；当时的 60 秒空闲回收已由 Session 常驻与上限回收替代 |
...
- `/:serverId/tools`：按配置版本保存工具启停和可信只读分类；定义指纹变化时旧可信分类失效，修改工具设置同时撤销该工具的持久授权。
...
| 2026-09-10 | MCP-015/037 | 服务器列表与详情优先使用通过来源与格式校验的 `serverInfo.icons`，尚未发现、缺失或加载失败时统一回退默认 MCP 图标，不根据名称或地址猜测品牌；设置导航继续使用 muted 的 MCP 图标；新增、导入与改名遇到重名 key 时自动使用 `-2`、`-3` 后缀 | Web 类型检查与 Biome；WEB-109/110 |
...
| 2026-09-10 | MCP-025/026/037 | 远程鉴权默认自动检测；401 challenge 区分 OAuth 与 Token/API Key；接入 SDK discovery、DCR、PKCE/state、回调换 token、串行 refresh；授权窗口复用插件的前端 HeroUI 启动/结果页并展示 MCP Logo | daemon/Web/agent-runtime 类型检查与受影响文件 Biome；真实第三方服务矩阵待执行 |
| 2026-09-10 | MCP-025 | DCR 显式注册 OAuth public client，修复授权服务器默认注册为 confidential client 后 token 交换缺少 client secret 的失败 | `example-server.modelcontextprotocol.io/mcp` 完整 discovery、DCR、PKCE、回调与 token 交换 |
| 2026-09-10 | MCP-007/013 | 启停或改名未改变连接配置时同步凭据 `configRevision`，避免开关往返后误报凭据与当前配置不匹配 | 同一 OAuth 服务器关闭、重新信任并连接验证；daemon 类型检查与受影响文件 Biome |
| 2026-09-12 | MCP-014/018/037 | 能力目录按配置版本持久化到 SQLite；启动、单台新增完成信任、重新启用和手动刷新更新目录；Run 改为只读本地快照，工具执行时才连接；设置页补齐连接 loading 与“刷新状态” | agent-runtime、daemon、Web 类型检查通过；未运行 dev/build/Git，浏览器验收待补 |
| 2026-09-10 | MCP-025/037 | OAuth 主页面改为比较脱敏 `credentialRevision`，只有本次授权写入新版本后才提示成功，避免旧凭据导致未授权即完成 | daemon/Web 类型检查与受影响文件 Biome；WEB-112 |
| 2026-09-10 | MCP-025/037 | OAuth 启动不再错误依赖工具连接信任；同一服务器的活动授权窗口唯一，重复点击只聚焦现有窗口 | 未信任且无凭据服务器启动接口由 403 变为 200 并到达服务商授权页；重复点击仍为同一浏览器窗口；daemon/Web 类型检查与 Biome；WEB-112 |
| 2026-09-10 | MCP-025/026/037 | OAuth 交互改为先启用、再开放授权；开关只反映启用状态，授权完成保留已有连接信任 | daemon/Web 类型检查与受影响文件 Biome；WEB-112 |
| 2026-09-12 | MCP-009/014/018/019/037 | Run 与工具调用不再强制重连/重扫目录，Session Client 保温并由 `list_changed` 失效；详情改为显式加载；接入工具级启停、可信只读与精确参数持久授权 | policy、agent-runtime、daemon、Web 类型检查通过；未运行 dev/build/Git；浏览器与运行时矩阵待验收 |
