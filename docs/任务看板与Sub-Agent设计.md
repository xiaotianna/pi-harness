...
| `review` | 待验收 | 紫色 | Run 正常完成，等待用户确认结果 |
| `completed` | 已完成 | 绿色 | 用户验收后手动标记完成 |

红色不作为普通看板列状态色，只用于真正的异常、失败或阻塞提示。

用户可以拖动任务或在详情中手动调整状态。Runtime 事件只自动推进到执行、等待或待验收；它不能替用户完成验收。
...
- 父任务只有在主执行和所有必需子执行进入终态后，才允许自动进入 `review`；其中任意必需分支需要用户处理时，父任务进入 `waiting`。
