...
| Tool 审批 | 底部审批卡使用 HeroUI `ButtonGroup` 组合“允许一次”和 `Dropdown`；非 Shell 普通操作可选“本次会话允许”。MCP 工具遵循全局策略：请求批准时人工审批、帮我批准时独立 AI 复核且不确定时询问、完全访问时自动批准；连接信任、账号鉴权及系统权限独立生效。“帮我批准”下 Shell 也由独立审批模型判断；模型返回 `ASK` 时只回退逐次人工审批，只有 Shell 同时提供且 daemon 校验通过 argv 前缀时显示“允许类似命令”，不提供“本次会话允许”。关键递归删除只允许逐次批准。`run_command` 被 SRT 明确阻止后，复用同一卡片展示原始原因、宿主权限风险与“在宿主机重试”单次操作，不提供会话或类似命令授权。菜单只显示操作名称，Web 只提交决策 | `features/chat/components/tool-approval-card.tsx`、`packages/policy/src/command-policy.ts` |
| 消息操作 | `ChatMessageActions` | `features/chat/components/message-actions.tsx` |
| 消息输入 | `PromptInput` 加 `ChatComposerEditor`；“+”菜单在同一级提供 Skills 与 MCP 子菜单，MCP 合并已启用且已信任的用户服务器与已安装插件中已开启的 App，并插入墨绿色提示标签；插件 App 候选和编辑器标签优先显示 App 图标或插件 Logo，普通服务器及无图标 App 回退通用 MCP 图标。读取菜单不得连接服务器或发现能力。Skill 标签使用紫色 | `features/chat/components/chat-composer.tsx`、`chat-composer-editor.tsx`、`chat-context-mention.tsx` |
...
| 任务中心 | HeroUI Pro `Kanban` 与 `useKanban`；使用一个全局看板聚合所有 Workspace，不按项目分组，也不提供 Workspace 筛选或搜索。五列固定表达待开始、执行中、需处理、待确认和已完成，Run 正常完成后进入待确认，用户确认后进入已完成；列反映真实执行生命周期，跨列拖拽只把“待开始 → 执行中”映射为启动任务、把“待确认 → 已完成”映射为确认完成，其他跨列拖拽不改写状态；任务详情使用只读状态展示，Footer 左侧以“更多”菜单收纳编辑和删除，右侧只保留当前主操作；删除只移除 BoardTask，保留关联 Session、Run 事件和工作区文件。每张卡片明确显示 Workspace 归属；无任务时继续展示五列及列内空列表提示，不使用整页 EmptyState；状态色固定为灰、蓝、橙、紫、绿，红色只用于异常或阻塞提示。“新建任务”作为页面主操作显示在 Navbar 搜索入口左侧，不在看板正文重复展示。未关联任务从详情直接后台创建 Session 并提交首个 Run，保持停留在看板；只有“进入对话”才导航到 Session。卡片对应顶层任务/Run，Sub Agent 只在任务详情中作为执行树分支展示 | `features/board/views/board-page.tsx`、`features/board/components/board-task-detail.tsx`、`docs/任务看板与Sub-Agent设计.md` |
...
| Workspace 文件浏览 | 会话页“文件”Tab 位于“对话/轨迹”之后；桌面端左侧使用 `react-resizable-panels` 与虚拟化目录树，右侧按需读取文件，预览工具栏在复制操作前提供带 Tooltip 的打开按钮并复用全局文件打开偏好；打开与复制按钮复用 CodeBlock 的 24px 图标按钮规格和 muted 浅色。移动端改为带返回入口的目录树与预览单栏切换且保留同一打开操作；文件复用 `FileIconRender` 的本地 VS Code 图标，UTF-8 文本复用 Shiki `CodeBlock` 渲染 | `features/chat/components/workspace-files-view.tsx`、`features/chat/api/workspace-queries.ts` |
...
| MCP 静态鉴权 | 远程表单在 URL 下使用 HeroUI `Fieldset` 与可增删的 Header 名称/值文本框；JSON 接受 `mcpServers.<id>.url` 同级的 `headers`，Streamable HTTP 可省略 `type`。Header 从连接配置输入后由 daemon 拆分到 Credential Store；服务器列表与详情只返回脱敏状态，打开编辑弹窗时通过禁止缓存的专用接口按需读取静态 Header/环境变量并保存在局部状态，不进入 TanStack Query 或浏览器持久化。静态鉴权不使用独立凭据编辑器；OAuth Token 不回显并保持独立授权流程 | `features/settings/components/mcp-server-editor.tsx`、`features/settings/components/mcp-settings-panel.tsx`、`features/settings/utils/mcp-form.ts`、`features/settings/api/mcp-api.ts`、`apps/daemon/src/services/mcp-server-service.ts` |
...
| 电脑操控设置 | 复用插件目录查询区分内置 Computer Use 插件未安装、应用未启用和已可用；页面以简短提示和插件目录同款设置项展示图标、名称与描述，未安装时就地安装、已安装时通过应用开关启停，不显示卸载；加载或读取失败时不显示猜测状态；系统权限与应用授权分区展示；系统权限两行始终可见、单项操作独立跟踪等待状态且不禁用另一项，首次读取和手动刷新期间行尾只显示加载，读取完成后才显示权限状态与操作；授权请求后对应状态保持“待核对”，仅在待核对时回到窗口或手动刷新才重新读取，普通窗口聚焦不刷新；每个权限标签的文案与语义色由同一逐项状态驱动，权限状态统一使用语义 soft Chip，行尾操作保持同一低强调 Button 层级；Web 用独立 TanStack Query 读取 daemon 返回的 macOS 辅助功能/屏幕录制实时状态，缺失时由明确的 HeroUI 操作请求 daemon 发起系统授权，最终确认仍由用户完成；系统权限两行使用分组底色，始终允许应用按设置目录行直接排列；始终允许应用继续按 bundle ID 管理，并通过 daemon 按需读取对应应用图标，在透明图标位以 40px 展示系统图标自身轮廓，失败时显示通用图标 | `features/settings/components/computer-use-settings-panel.tsx`、`hooks/use-computer-use-permissions.ts` |
...
| 插件 App、Skill 与图标 | 插件可同时包含多个 `.app.json` MCP App 和多个 `manifest.yaml` Skill；App item 只显示自身图标、描述和尾部连接操作，点击后进入与 MCP 服务器详情共用页面骨架的应用详情，并复用 MCP 详情的完整能力目录组件展示工具、资源与模板、提示词、服务端信息、连接错误、刷新操作和工具级设置；已启用的独立 OAuth App 缺少凭据时，列表尾部直接显示主“授权”按钮并启动与详情相同的流程，其他状态显示状态按钮，Switch 与 Token/API Key 配置只放在详情中。App 复用现有 MCP 执行链但不出现在 MCP 设置列表。缺少声明所需的 OAuth 或静态凭据时，详情优先显示“未授权”或“未配置凭据”，只有授权前置条件满足后的异常才显示“连接失败”。只有官方接入说明明确接受所属插件现有 access token、且授权范围覆盖 App 能力的内置 App，才可显式声明共用插件 OAuth；daemon 在 MCP 请求前使用插件 OAuth 刷新链路读取同一 token，不另存凭据副本，连接、刷新与断开状态均跟随插件授权；刷新凭据被服务商明确拒绝或 MCP 返回未授权时删除共享凭据，Web 同步提示重新授权。插件 Logo 默认读取 `logo.svg`，也可由 `manifest.yaml` 的 `logo` 声明同目录内的 SVG、PNG、JPEG、WebP 或 GIF；插件、App 与 Skill 的列表及详情中，位图 Logo 铺满容器，SVG 品牌标记保留统一安全边距；App 与 Skill 优先使用各自声明的本地 SVG，未声明时由 daemon 回退插件 Logo，再由 Web 回退对应通用图标 | `packages/tools/src/plugins/load-plugin.ts`、`features/settings/components/plugin-marketplace-panel.tsx`、`features/settings/components/mcp-server-detail.tsx` |
