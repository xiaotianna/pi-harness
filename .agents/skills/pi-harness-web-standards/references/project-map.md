...
| Workspace 文件浏览 | 会话页“文件”Tab 位于“对话/轨迹”之后；桌面端左侧使用 `react-resizable-panels` 与虚拟化目录树，右侧按需读取文件，移动端改为带返回入口的目录树与预览单栏切换；文件复用 `FileIconRender` 的本地 VS Code 图标，UTF-8 文本复用 Shiki `CodeBlock` 渲染 | `features/chat/components/workspace-files-view.tsx`、`features/chat/api/workspace-queries.ts` |
