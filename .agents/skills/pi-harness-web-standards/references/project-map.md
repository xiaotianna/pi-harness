...
| Run 运行反馈 | 当前 Session 存在活动 Run 时，在会话消息列表末尾追加使用主题 accent 蓝色的 HeroUI Pro `TextShimmer` “正在处理…”，跟随最新消息滚动，Run 终态后隐藏 | `features/chat/utils/session-messages.ts`、`features/chat/components/thread-message/loading-thread-message.tsx` |
