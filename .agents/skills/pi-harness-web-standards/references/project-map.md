...
| Workspace 文件浏览 | 会话页“文件”Tab 位于“对话/轨迹”之后；左侧使用 `react-resizable-panels` 与虚拟化目录树，文件复用 `FileIconRender` 的本地 VS Code 图标，右侧按需读取单个 UTF-8 文本文件并复用 Shiki `CodeBlock` 渲染 | `features/chat/components/workspace-files-view.tsx`、`features/chat/api/workspace-queries.ts` |
