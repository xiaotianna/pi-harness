...
- 使用 Gravity UI Icons 表达普通界面图标；没有语义等价图标时才使用 Lucide React。图标继承现有 HeroUI 语义色和业务状态 class；同级操作显式保持一致的图标视觉尺寸与颜色。填充路径图标不支持 `strokeWidth` 时，通过尺寸和语义色控制视觉重量，不依赖容器默认 SVG 尺寸。使用 `ModelProviderIcon` 表达 Provider 品牌。
