...
import { Alert, Button, ScrollShadow, Skeleton } from "@heroui/react";
...
function WorkspaceFilePreview({ content, path }: { content: string; path: string }) {
  return (
...
export function WorkspaceFilesView({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    if (selectedPath !== null || !filesQuery.data) return;
    const firstFile = filesQuery.data.items.find(
      (item) => item.kind !== UserContextReferenceKind.FOLDER,
    );
    if (!firstFile) return;
    const segments = firstFile.path.split("/");
    setExpandedPaths(
      new Set(segments.slice(0, -1).map((_, index) => segments.slice(0, index + 1).join("/"))),
    );
  ...
  if (filesQuery.isPending) {
    return (
      <div aria-busy className="grid h-full grid-cols-[200px_1fr] px-8" role="status">
        <span className="sr-only">正在加载 Workspace 文件</span>
        <div className="space-y-2 border-r border-separator p-3">
          {Array.from({ length: 8 }, (_, index) => (
            <Skeleton aria-hidden className="h-8 rounded-md" key={index} />
          ))}
        </div>
      ...
      <Panel id="tree" defaultSize="200px" minSize="200px" maxSize="45%">
        <aside className="flex h-full min-w-0 flex-col bg-background" aria-label="文件目录树">
          ...
          {visibleNodes.length === 0 ? (
            <div className="flex min-h-0 flex-1 items-center justify-center px-4 text-sm text-muted">
              Workspace 中没有可展示的文件
            </div>
          ) : (
            <ScrollShadow
            ...
            >
              <div className="relative w-full" style={{ height: virtualizer.getTotalSize() }}>
                {virtualizer.getVirtualItems().map((virtualItem) => {
                  ...
                  return (
                    <div
                      className="absolute top-0 right-0 left-0 px-1"
                      key={virtualItem.key}
                      style={{
        ...
        <section className="h-full min-w-0 bg-background" aria-label="文件内容">
          {selectedPath === null ? (
            ...
            <div className="flex h-full items-start justify-center p-6">
              <Alert status="danger">
                <Alert.Content>
                  <Alert.Title>无法预览此文件</Alert.Title>
                  <Alert.Description>{fileQuery.error.message}</Alert.Description>
                </Alert.Content>
              </Alert>
            </div>
          ) : fileQuery.data ? (
            <WorkspaceFilePreview content={fileQuery.data.content} path={fileQuery.data.path} />
          ) : null}
        </section>
      </Panel>
    </Group>
