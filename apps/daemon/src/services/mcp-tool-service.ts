import type { PreparedExternalTools } from "@pi-harness/agent-runtime";
      ...
      for (const server of this.servers.list()) {
        ...
        for (const tool of catalog.tools) {
          ...
          const verify = async (args: unknown, requestSignal: AbortSignal): Promise<void> => {
            ...
            const current = await this.discovery.discover(lease, requestSignal, true);
            const currentTool = current.tools.find((item) => item.name === tool.name);
            if (
              current.generation !== lease.catalogGeneration ||
              !currentTool ||
