...
URL-encode query values. Match by name, MIME type, parent, or modified time, then summarize the strongest results. The emulator exposes metadata APIs only, so do not claim to have read document contents.
