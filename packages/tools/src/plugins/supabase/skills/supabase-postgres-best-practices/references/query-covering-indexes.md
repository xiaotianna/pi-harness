...
Covering indexes contain the required columns and can enable index-only scans. Heap access may still be needed for visibility checks; inspect Heap Fetches and the visibility map.
...
-- All columns are available in the index; verify actual heap fetches
select email, name, created_at from users where email = 'user@example.com';
```
